...
def source_set_setting(unused_addr, source_name, setting, value, private=False):
	...
	with ExitStack() as stack:
		...
		if source:
			newSettings = stack.enter_context(new_settings())
			if   type(value) == bool:  obs.obs_data_set_bool(newSettings, setting, value)
			...
			elif type(value) == str:   obs.obs_data_set_string(newSettings, setting, value)
			elif type(value) == int:   obs.obs_data_set_int(newSettings, setting, value)
			elif type(value) == float: obs.obs_data_set_double(newSettings, setting, value)
...
def source_assign_filter(unused_addr, source_name, filter_type='color_filter', filter_name="Color Correction", private=False):
	c4(f'source_assign_filter(source_name="{source_name}", filter_type="{filter_type}", filter_name="{filter_name}")')
...
def item_duplicate(unused_addr, 
	...
	with source_list_auto_release(scenes_list1):
		if item_name.startswith('id:'): scene_item = obs.obs_scene_find_sceneitem_by_id(fromScene, int(item_name[3:]))
		else: scene_item = obs.obs_scene_find_source(fromScene, item_name)
		...
		if scene_item:
			...
			if tobool(reference):
				...
				# Duplica
				duplicated = obs.obs_source_duplicate(source, new_item_name, tobool(private))
				print(duplicated)
				# Crea item con 
